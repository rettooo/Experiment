...
class CareerHYResponseGenerator(BaseResponseGenerator):
    ...
    async def generate(
        ...
        try:
            ...
            logger.info(f"  - recommended_job_indices: {result.recommended_job_indices}")
            logger.info(f"  - overall_advice 길이: {len(result.overall_advice)}")
            logger.info(f"  - recommendation_reasons 개수: {len(result.recommendation_reasons)}")
            ...
            try:
                ...
                if 0 <= doc_index < len(retrieved_docs):
                    ...
                    recommended_job = RecommendedJob(
                        ...
                        title=metadata.get('title', metadata.get('post_title', '제목 없음')),
                    ...
                    logger.warning(f"❌ 잘못된 추천 인덱스: {job_index} (범위: 1-{len(retrieved_docs)})")
...
하지만 검색된 채용공고들을 바탕으로 관련성이 높은 공고들을 추천드립니다."""
        ...
        for i, doc in enumerate(retrieved_docs[:3]):
            ...
            recommended_job = RecommendedJob(
                ...
                title=metadata.get('title', metadata.get('post_title', f'채용공고 {i+1}')),
                url=metadata.get('url', ''),
                deadline=metadata.get('deadline'),
