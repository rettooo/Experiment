...
class TextJobPostParser:
    """
    순수 텍스트 기반 채용 공고 파서

    - S3DataLoader에서 이미 추출/정제된 텍스트 문자열을 입력으로 사용
    - unstructured에 의존하지 않고, 라인/블록 단위로 직접 섹션 감지
    - 출력 형식은 JobPostParser.process_document와 동일 (청크 딕셔너리 리스트)
    """
    ...
    def __init__(
        ...
        print(f"✅ TextJobPostParser 초기화:")
        print(f"   - 컨텍스트 주입: {include_context}")
        print(f"   - 최소 섹션 길이: {min_section_length}자")

    def _normalize_text(self, text: str) -> str:
        """
        텍스트 정규화 (JobPostParser._normalize_text 와 동일한 규칙)
        """
        normalized = re.sub(r"^[\[\]()■◆●▶◾▪*\s]+", "", text)
        normalized = normalized.strip().strip(" *-_[]()!?")
        return normalized

    def _detect_section_text(self, text: str) -> Optional[str]:
        """
        한 블록(문단) 텍스트에서 섹션 타입 감지
        """
        if not text:
            return None
    ...
    def _split_into_blocks(self, text: str) -> list:
        """
        전체 텍스트를 블록(문단) 리스트로 분리

        규칙:
        - 빈 줄(공백만 있는 줄) 기준으로 문단 구분
        - "#태그"로 시작하는 라인은 항상 별도 블록으로 유지
        - 페이지 구분 라인(--- Page N ---)은 버리거나 무시
        """
    ...
    def _group_by_sections_from_blocks(
        self, blocks: list
    ) -> Tuple[Dict[str, list], List[str]]:
        """
        블록 리스트를 섹션별로 그룹화 + 태그 감지
        """
        ...
        for block in blocks:
            ...
            # 섹션 감지
            detected_section = self._detect_section_text(text)
            if detected_section:
                current_section = detected_section
                print(f"   🔖 섹션 감지: {current_section} - '{text[:40]}...'")
    ...
    def _sections_to_chunks_from_blocks(
        self, sections: Dict[str, list], metadata: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """
        섹션별 블록 리스트를 청크로 변환
        (JobPostParser._sections_to_chunks 와 거의 동일, 입력 타입만 다름)
        """
        chunks = []
    ...
    def process_text(
        self, text: str, original_metadata: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """
        메인 오케스트레이터: 순수 텍스트 → 섹션 그룹화 → 청크 생성

        Args:
            text: 전체 채용 공고 텍스트 (S3DataLoader에서 추출/정제된 텍스트)
            original_metadata: 원본 메타데이터
                - rec_idx (필수)
                - company (필수)
                - title (필수)
        ...
        print(f"\n{'='*70}")
        print(f"📄 [TEXT] 문서 처리: {original_metadata.get('rec_idx')}")
        print(f"   회사: {original_metadata.get('company')}")
        print(f"   직무: {original_metadata.get('title')}")
        print(f"{'='*70}")
        ...
        # 2) 섹션별 그룹화 + 태그 추출
        sections, tags = self._group_by_sections_from_blocks(blocks)
        print(f"✅ {len(sections)}개 섹션 감지")
        print(f"✅ {len(tags)}개 태그 추출: {tags[:5]}...")
        ...
        # 4) 섹션 → 청크 변환
        chunks = self._sections_to_chunks_from_blocks(sections, doc_metadata)
        print(f"✅ {len(chunks)}개 청크 생성")
