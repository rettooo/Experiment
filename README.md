...
| 항목 | 기존 GT (v1) | 새로운 GT (v2) |
|------|-------------|---------------|
| **정답 문서 수** | 고정 5개 | 가변 크기 (평균 19.8개) |
| **생성 방식** | 수동/단순 매칭 | 클러스터 기반 자동 생성 |
| **신뢰성** | 낮음 (제한적 샘플) | 높음 (클러스터 전체 포함) |
| **GT 범위** | 제한적 | 클러스터 내 모든 문서 |
| **최대 GT 수** | 5개 | 57개 |
| **최소 GT 수** | 5개 | 1개 |

#### 주요 개선 사항
- **클러스터 기반 GT**: 태그 정보를 이용한 정답 군집 생성으로 신뢰성 향상
- **가변 크기 지원**: 쿼리별로 적절한 수의 정답 문서 제공
- **자동화**: GT 생성 파이프라인을 통한 자동 생성
